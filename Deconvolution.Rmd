...
```{r}
deconvolve_single_curve = function(curve, params)
{
  #simple
  simple=deconvolve_infection_curve_simple(curve,params[1])
  #random
  random=deconvolve_infection_curve_random(curve,generate_incubation_period, trials=100, distribution="gamma", parms=parms)
  #ridge
  ...
  curves = list(curve, simple, random, ridge, rl, frequency, frequency, average)
  return(curves)
}
results = deconvolve_single_curve(curve = outbreak_cov, params = params)
```
...
#params = c(newmean, newsd)
#nextresults = deconvolve_single_curve(curve = newcurve, params = params)
